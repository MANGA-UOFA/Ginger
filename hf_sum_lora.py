...
def main(cfg):
    ...
    def decision_fn(path, param):
        result = lora.LORA_FREEZE

        if "encoder_attn" in path or "self_attn" in path:
            # BART
            result = cfg.training.lora_dim

        if "attn" in path:
            # GPT2
            if "attn.c_attn" in path:
                result = cfg.training.lora_dim * 3
            elif "attn.c_proj" in path:
                result = cfg.training.lora_dim

        if "Attention." in path and "relative_attention_bias" not in path:
            # T5
            result = cfg.training.lora_dim
    ...
    lora_spec = lora.simple_spec(model.params, decision_fn=decision_fn, tune_vectors=True)

    lora_params, freezed = lora.init_lora(model.params, lora_spec, rng)

    def merge_params(params, freezed):
        lora_params = flax.traverse_util.flatten_dict(params, sep=".")
        full_params = {}
        for path, spec in lora_spec.items():
            if spec == lora.LORA_FULL:
                full_params[path] = lora_params[path]
            elif spec == lora.LORA_FREEZE:
                full_params[path] = freezed[path]
            else:
                a = lora_params[f"{path}.lora.a"]
                b = lora_params[f"{path}.lora.b"]
                alpha = freezed[f"{path}.lora.alpha"]
                full_params[path] = jnp.matmul(a, b) * alpha / spec + freezed[path]
        return flax.traverse_util.unflatten_dict(full_params, sep=".")

    def apply_fn(params, freezed, *args, **kwargs):
        return model(params=merge_params(params, freezed), *args, **kwargs)
    ...
    state = TrainState.create(
        apply_fn=apply_fn, params=lora_params, tx=optimizer, dropout_rng=dropout_rng, freezed=freezed
    )
    ...
    def train_step(state, batch, label_smoothing_factor=0.0):
        gacc_steps = batch["input_ids"].shape[0]

        def compute_loss(params, inputs, rng):
            labels = inputs.pop("labels")
            logits = state.apply_fn(**inputs, params=params, dropout_rng=rng, train=True, freezed=state.freezed)[0]
        ...
        def scan_body(carry, step_inputs):
            ...
            loss = loss + step_loss / gacc_steps
            grad = jax.tree_map(lambda x, y: x + y / gacc_steps, grad, step_grad)
            ngrad = jax.tree_map(lambda x, y: x + y / gacc_steps, ngrad, step_ngrad)
    ...
    def eval_step(state, batch, label_smoothing_factor=0.0):
        ...
        logits = apply_fn(**batch, params=state.params, freezed=state.freezed, train=False)[0]
        loss, num_labels = loss_fn(logits, labels, batch["decoder_attention_mask"], label_smoothing_factor)
    ...
    def generate_step(state, batch):
        model.params = merge_params(state.params, state.freezed)
...
@hydra.main(version_base=None, config_path="configs", config_name="sum_lora")
